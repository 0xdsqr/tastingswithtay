...
const statusConfig: Record<string, { label: string; color: string; icon: typeof Beaker }> = {
  in_progress: {
    label: "In Progress",
...
function ExperimentCard({ experiment }: { experiment: Experiment }): React.ReactElement {
  const config = statusConfig[experiment.status] ?? statusConfig.in_progress!
  const StatusIcon = config!.icon

  return (
    <Link to="/test-kitchen/$slug" params={{ slug: experiment.slug }} className="group block">
        ...
        <section className="relative overflow-hidden py-24 lg:py-32">
          ...
          <div className="relative z-10 mx-auto max-w-7xl px-6 text-center lg:px-8">
            ...
            <p className="mx-auto max-w-2xl text-lg leading-relaxed text-muted-foreground">
              Where recipes are born. Follow along as I experiment, iterate, and discover new
              flavors. Some of these will become full recipes -- others are just delicious learning
              experiences.
            </p>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            ...
            <div className="mb-12 flex flex-wrap justify-center gap-3">
              {statusFilters.map((status) => {
                const isActive =
                  status === "All" ? activeStatus === "All" : statusMap[status] === activeStatus
                return (
                  <a
                    key={status}
                    href={
