...
export const Route = createFileRoute("/wine/$slug")({
  loader: async ({ context, params }) => {
    const [wine, relatedWines] = await Promise.all([
      context.queryClient.fetchQuery(context.trpc.wines.bySlug.queryOptions({ slug: params.slug })),
      context.queryClient.fetchQuery(context.trpc.wines.list.queryOptions({ limit: 3 })),
    ])
    if (!wine) {
      throw notFound()
    }
          ...
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
              ...
              <div className="flex flex-col justify-center">
                ...
                <p className="mb-6 text-xl text-muted-foreground">{wine.winery}</p>
                  ...
                  <div className="mb-8 flex items-center gap-2">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`h-6 w-6 ${
                          i < (wine.rating ?? 0) ? "fill-primary text-primary" : "text-muted"
                        }`}
                      />
                    ))}
                    <span className="ml-2 text-sm text-muted-foreground">Tay's Rating</span>
                  </div>
                )}
                  ...
                  {wine.grapes && (
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-foreground">{wine.grapes}</p>
                      </div>
                    </div>
                  )}
                  {wine.priceRange && (
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-foreground">{wine.priceRange}</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {wine.notes && (
                  <div className="mb-8">
                    <h2 className="mb-3 font-serif text-xl text-foreground">Tasting Notes</h2>
                    <p className="leading-relaxed text-foreground/80">{wine.notes}</p>
                  </div>
                )}
                ...
                {wine.aromas && wine.aromas.length > 0 && (
                  <div className="mb-8">
                    <h2 className="mb-3 font-serif text-xl text-foreground">Aromas</h2>
                ...
                {wine.pairings && wine.pairings.length > 0 && (
                  <div>
                    <h2 className="mb-3 font-serif text-xl text-foreground">Perfect Pairings</h2>
                    <div className="flex flex-wrap gap-2">
