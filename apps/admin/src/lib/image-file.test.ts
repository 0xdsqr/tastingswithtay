import { describe, expect, it } from "vitest"
import { detectSupportedImageType } from "./image-file"

describe("detectSupportedImageType", () => {
  it.each([
    ["JPEG", [0xff, 0xd8, 0xff, 0xe0], { contentType: "image/jpeg", extension: "jpg" }],
    [
      "PNG",
      [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a],
      { contentType: "image/png", extension: "png" },
    ],
    [
      "WebP",
      [0x52, 0x49, 0x46, 0x46, 0, 0, 0, 0, 0x57, 0x45, 0x42, 0x50],
      { contentType: "image/webp", extension: "webp" },
    ],
    [
      "AVIF",
      [0, 0, 0, 24, 0x66, 0x74, 0x79, 0x70, 0x61, 0x76, 0x69, 0x66],
      { contentType: "image/avif", extension: "avif" },
    ],
  ])("detects %s by file signature", (_label, input, expected) => {
    expect(detectSupportedImageType(Uint8Array.from(input as number[]))).toEqual(expected)
  })

  it("rejects content that only claims to be an image", () => {
    expect(detectSupportedImageType(new TextEncoder().encode("not an image"))).toBeNull()
  })
})
